    ...
    service {
      ...
      connect {
        sidecar_service {
          proxy {
            upstreams {
              # By configuring an upstream destination to
              # the linked service of the terminating gateway, the dashboard is able to make
              # requests through the gateway to the count-api service.
