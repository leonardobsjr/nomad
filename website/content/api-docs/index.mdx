...
in follow-up request to retrieve the next page.

When the last page is reached, the `X-Nomad-Nexttoken` HTTP header will not
be present in the response, indicating that there is no more data to request.

## Ordering

Results are often returned by sorted by their internal key order. Some
list endpoints allow returning the data sorted by their Raft `CreateIndex`,
which roughly corelates to their creation order. The order may be reversed
using the `reverse` query parameter.
